    ...
    <AppLayout :breadcrumbs="[{ title: 'Administración', href: '#' }, { title: 'Carreras', href: '/admin/carreras' }]">
        ...
        <main class="flex flex-col gap-6 p-4 md:p-8 max-w-7xl mx-auto w-full animate-in fade-in duration-500">
            ...
            <section aria-live="polite">
                <div v-if="isEditing" class="bg-muted/30 border rounded-xl p-6 shadow-sm mb-6 animate-in slide-in-from-top-2">
                    ...
                    <form @submit.prevent="submit" class="grid gap-6 md:grid-cols-12 items-end">
                        ...
                        <div class="space-y-2 md:col-span-6">
                            <Label for="nombre-carrera">Nombre de la Carrera <span class="text-destructive">*</span></Label>
                            <Input 
                                id="nombre-carrera"
                                ref="inputNombreRef"
                                v-model="form.nombre" 
                        ...
                        <div class="space-y-2 md:col-span-3">
                            <Label for="codigo-carrera">Código <span class="text-xs text-muted-foreground">(Único)</span> <span class="text-destructive">*</span></Label>
                            <Input 
                                id="codigo-carrera"
                                v-model="form.codigo" 
            ...
            <section aria-label="Listado de carreras">
                    ...
                    <table class="w-full caption-bottom text-sm">
                        ...
                        <thead class="bg-muted/50 [&_tr]:border-b">
                            <tr class="border-b transition-colors">
                                <th scope="col" class="h-12 px-4 text-left align-middle font-medium text-muted-foreground w-[150px]">Código</th>
                                <th scope="col" class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Nombre de la Carrera</th>
                                <th scope="col" class="h-12 px-4 text-right align-middle font-medium text-muted-foreground w-[120px]">Acciones</th>
                            </tr>
                        </thead>
                        <tbody class="[&_tr:last-child]:border-0">
                            <tr v-for="item in carreras.data" :key="item.id" class="border-b transition-colors hover:bg-muted/50 group">
                                ...
                                <td class="p-4 align-middle text-right">
                                    <div class="flex justify-end gap-1 opacity-100 transition-opacity">
                                        <Button 
                                            ...
                                            size="icon" 
                                            @click="openEdit(item)"
                                            :aria-label="`Editar carrera ${item.nombre}`"
                                            title="Editar"
                                        >
                                        ...
                                        <Button 
                                            ...
                                            :aria-label="`Eliminar carrera ${item.nombre}`"
                                            title="Eliminar"
                            ...
                            <div class="flex justify-between items-start">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs font-mono text-muted-foreground bg-muted inline-block px-1.5 rounded mt-1">
                                            {{ item.codigo }}
                                        </p>
                                    </div>
                                </div>
                ...
                <div v-if="carreras.data.length === 0" class="flex flex-col items-center justify-center py-12 text-center border-2 border-dashed rounded-xl bg-muted/20">
